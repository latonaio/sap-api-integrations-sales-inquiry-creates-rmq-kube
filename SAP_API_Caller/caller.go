...
import (
	...
	"sap-api-integrations-sales-inquiry-creates-rmq-kube/SAP_API_Caller/responses"
...
func (c *SAPAPICaller) AsyncPostSalesInquiry(
...
func (c *SAPAPICaller) HeaderItem(headerItem *requests.HeaderItem) {
	err := c.callSalesInquirySrvAPIRequirementHeaderItem("A_SalesInquiry", headerItem)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerItem, "function": "SalesInquiryHeaderItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSalesInquirySrvAPIRequirementHeaderItem(api string, headerItem *requests.HeaderItem) error {
	...
	headerItem.SalesInquiry = resBody.D.SalesInquiry
	return nil
}
